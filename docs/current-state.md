...
- **Tagged releases + Homebrew tap.** CI is in place (build + vet matrix on every push/PR); the next step is `.github/workflows/release.yml` triggered on `v*` tag push that attaches binaries to a GitHub Release, and a shared `roselabs-io/homebrew-tools` tap.
- **Real unit tests.** `go test ./...` is wired into CI as a placeholder. Worth covering once integration-test patterns settle.
